...
class path_planner(Node):
    ...
    def get_map(self, data):
        ...
        for cone in data.blue_cones:
            if self.distance(cone.point.x, cone.point.y)<4.5:
    ...
    def get_waypoints(self):
        ...
        number_blue=0
        number_yellow=0
        for cone in bluecones_withcolor:
            marker = Marker()
            marker.ns = "track"
            marker.type = 2
            marker.action = 0
        ...
        for cone in yellowcones_withcolor:
            marker = Marker()
            marker.ns = "track"
            marker.type = 2
            marker.action = 0
        ...
        for triangle in new_list3:
            for i in [0,1]:
                for j in range(i+1,3):
                    ...
                    if 2.9<line_length<4 and triangle[i][2]!=triangle[j][2]:
